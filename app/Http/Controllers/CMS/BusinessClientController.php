...
class BusinessClientController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('cms.clients.index', compact('pages'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (BusinessClient::where('client_name', $request->client_name)->exists()) {
            flash()->addError('Sorry, Duplicate Found, Business Client save operation has been failed.');
        ...
        $client = BusinessClient::create([
            ...
            'slug'        => generateSlug($request->client_name),
    ...
    public function edit($id)
    {
        ...
        $client  = BusinessClient::findOrFail($keyId);
        $blog_categories = DB::table('blog_categories')
                    ->select('*')
                    ->orderBy('id','DESC')
                    ->get();

        return view('cms.clients.edit', compact('client','blog_categories'));
    }
    ...
    {
        $business_clients = DB::table('business_clients')->select(['*'])->orderBy('id', 'DESC')->get();
        ...
        return datatables()->of($business_clients)
            ...
            ->addColumn('action', function ($business_clients) {
                $btn2 = '<a href="' . '/client/edit/' . Crypt::encryptString($business_clients->id) . '" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Edit</a> ';
                return $btn2;
            })
